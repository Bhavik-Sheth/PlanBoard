"""
planner/tools/file_tools.py — File manipulation and scaffolding tools.
"""
...
from planner.tools.exceptions import OverwriteProtectionError, ReadOnlyFileError
...
def scaffold_planner(project_path: str) -> list[str]:
    """
    ...
    # Create directories
    for directory in (planner_dir, diagrams_dir, modules_dir):
...
def list_planner_files(project_path: str) -> dict[str, dict]:
    """
    Returns all files in PLANNER/ with metadata: {path: {size, modified_at, is_empty}}
    The 'path' key is the relative path from the PLANNER/ directory (e.g. 'PRD.md').
    """
    planner_dir = Path(project_path) / "PLANNER"
    result = {}
    
    if not planner_dir.exists() or not planner_dir.is_dir():
        return result
        
    for root, _, files in os.walk(planner_dir):
        for file in files:
            full_path = Path(root) / file
            # Relative path from PLANNER/
            rel_path = str(full_path.relative_to(planner_dir))
