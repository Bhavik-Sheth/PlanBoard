...
DATA_FLOW_SYSTEM_PROMPT = """You are an expert Software Architect AI.
Your only goal is to generate a comprehensive, clear, and professional `DataFlow.md` file (detailing how data flows through the system, including a HORIZONTAL ASCII diagram of key data flows inside a markdown code block, and a brief explanation below) based on Structured Idea, Constraints, PRD, TRD, Schema, DesignDecisions, AppFlow.

Specifications:
- The output must contain a HORIZONTAL ASCII diagram inside a markdown code block detailing key data flows through the system.
...
def _gather(state: PlannerState) -> PlannerState:
    ...
    if not structured_idea_path.exists():
        ...
        state.current_file = "ARCHITECTURE_DIAGRAMS/DataFlow.md"
        state.calling_agent = "data_flow"
        return state
...
def _write(state: PlannerState) -> PlannerState:
    ...
    update_inst = get_update_instructions(state, "ARCHITECTURE_DIAGRAMS/DataFlow.md")
    if update_inst:
        user_content += update_inst

    messages = [
        SystemMessage(content=DATA_FLOW_SYSTEM_PROMPT),
    ...
    filepath = diagrams_dir / "DataFlow.md"
...
def data_flow_agent(state: PlannerState) -> PlannerState:
    """
    Data Flow Agent node: reads StructuredIdea.md, PRD, TRD, Schema, etc.,
    generates DataFlow.md and updates state.
    """
