...
across all PLANBOARD/ files.  Returns a list of issues citing both files
involved. Never modifies any file.

Inputs:
    files: dict[str, str]   — all non-empty PLANBOARD/ files {filename: content}
...
from planboard.tools import llm_call_json
...
def consistency_agent(files: dict[str, str]) -> dict:
    """
    Perform a read-only cross-file consistency check across PLANBOARD/ files.

    Args:
        files: Mapping of ``{filename: content}`` for every non-empty
               PLANBOARD/ file (e.g. ``{"PRD.md": "...", "TRD.md": "..."}``).
