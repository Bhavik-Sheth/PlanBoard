...
from planboard.state import PlannerState, save_state
from planboard.agents._base import invoke_llm_safe, strip_markdown_fence, load_context
...
from planboard.tools.validation_tools import REQUIRED_SECTIONS_MAP
...
def _file_is_populated(planboard_dir: Path, filename: str) -> bool:
    """Return True if the target file exists and is non-empty."""
    path = planboard_dir / filename
...
class OrchestratorAgent:
    ...
    def __init__(self, state: PlannerState) -> None:
        self.state = state
        self.planboard_dir = Path(state.project_path)
        from planboard.watcher.watcher_manager import WatcherManager
        self.watcher_manager = WatcherManager(str(self.planboard_dir))
    ...
    def _update_tracker(
        ...
        # tracker_tools.update_file_status uses the *project root* (parent of PLANBOARD/)
        project_root = str(self.planboard_dir.parent)
        try:
    ...
    def handle_init(self) -> dict:
        """Scaffold PLANBOARD/ directory."""
        project_root = str(self.planboard_dir.parent)
        scaffold_planboard(project_root)
        initialize_tracker(project_root, _TRACKER_SEQUENCE)
        self.watcher_manager.start()
        return self._payload("ready", message="PLANBOARD/ created.")
    ...
    def handle_describe(self, text: str) -> dict:
        ...
        raw_idea_path = self.planboard_dir / "RawIdea.md"
        append_file(str(raw_idea_path), text)

        # Route to StructuringAgent
        from planboard.agents.structuring_agent import run_structuring
    ...
    def handle_run(self) -> dict:
        ...
        """
        lock_path = self.planboard_dir / ".graph_running"
        try:
    ...
    def handle_revise(self, target: str, request: str) -> dict:
        ...
        file_path = self.planboard_dir / target
    ...
    def handle_reset_confirmed(self, file: str) -> dict:
        ...
        file_path = self.planboard_dir / file
        if file_path.exists():
    ...
    def handle_consistency(self) -> dict:
        ...
        from planboard.agents.consistency_agent import consistency_agent

        # Load all PLANBOARD/ files
    ...
    def handle_finalize(self) -> dict:
        ...
        for _, target_file in _SEQUENCE:
            if target_file not in self.state.approved_files:
                if _file_is_populated(self.planboard_dir, target_file):
    ...
    def _finalize_execute(self) -> dict:
        ...
        for fname in _CONSISTENCY_FILES:
            fpath = self.planboard_dir / fname
            if fpath.exists():
    ...
    def _commit_update(self) -> dict:
        ...
        from planboard.tools.tracker_tools import append_change_log
        project_root = str(self.planboard_dir.parent)
        ...
        max_idx = -1
        from planboard.agents.orchestrator import _SEQUENCE
    ...
    def handle_update(self, text: str) -> dict:
        ...
        # 1. Load all current PLANBOARD/ file contents + tracker_state into memory
        all_files = {}
    ...
    def handle_get_suggestion(self, question: str) -> dict:
        ...
        from planboard.agents.tech_stack_agent import run as run_tech_stack
...
_AGENT_REGISTRY: dict[str, str] = {
    "structuring":    "planboard.agents.structuring_agent.structuring_agent",
    "constraints":    "planboard.agents.constraints_agent.constraints_agent",
    "prd":            "planboard.agents.prd_agent.prd_agent",
    "trd":            "planboard.agents.trd_agent.trd_agent",
...
def orchestrator(state: PlannerState) -> PlannerState:
    ...
    if state.current_file and state.current_file not in state.approved_files:
        planboard_dir = Path(state.project_path)
        path = planboard_dir / state.current_file
        if path.exists() and path.stat().st_size > 0:
    ...
    print("\n✅  All planning files are complete! "
          "Run `planboard finalize` to generate CLAUDE.md.\n")
    save_state(state)
    return state
...
def run_mode_selection(state: PlannerState) -> PlannerState:
    ...
    if choice == "1":
        ...
        raw_idea = "\n".join(lines).strip()
        raw_idea_path = planboard_dir / "RawIdea.md"
