...
SYSTEM_DESIGN_SYSTEM_PROMPT = """You are an expert Software Architect AI.
Your only goal is to generate a comprehensive, clear, and professional `SystemDesign.md` file (which shows the core design patterns or system interactions using a HORIZONTAL ASCII diagram inside a markdown code block, and a brief explanation below) based on Structured Idea, Constraints, PRD, TRD, Schema, DesignDecisions, AppFlow.

Specifications:
- The output must contain a HORIZONTAL ASCII diagram inside a markdown code block showing the core design patterns or system interactions.
...
def _gather(state: PlannerState) -> PlannerState:
    ...
    if not structured_idea_path.exists():
        ...
        state.current_file = "ARCHITECTURE_DIAGRAMS/SystemDesign.md"
        state.calling_agent = "system_design"
        return state
...
def _write(state: PlannerState) -> PlannerState:
    ...
    update_inst = get_update_instructions(state, "ARCHITECTURE_DIAGRAMS/SystemDesign.md")
    if update_inst:
        user_content += update_inst

    messages = [
        SystemMessage(content=SYSTEM_DESIGN_SYSTEM_PROMPT),
    ...
    filepath = diagrams_dir / "SystemDesign.md"
...
def system_design_agent(state: PlannerState) -> PlannerState:
    """
    System Design Agent node: reads StructuredIdea.md, PRD, TRD, Schema, etc.,
    generates SystemDesign.md and updates state.
    """
