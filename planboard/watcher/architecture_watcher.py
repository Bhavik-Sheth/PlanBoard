# planboard/watcher/architecture_watcher.py
import sys
import os
import time
import asyncio
...
from planboard.watcher.heartbeat import write_heartbeat
...
async def main(project_path_str: str):
    ...
    # We watch the PLANBOARD/ directory itself, but ignore changes in ARCHITECTURE_DIAGRAMS/
...
if __name__ == "__main__":
    ...
    # Path to PLANBOARD directory passed as argument
    planboard_dir = sys.argv[1]
    
    try:
        asyncio.run(main(planboard_dir))
    except (KeyboardInterrupt, SystemExit):
        pass
    except Exception:
        # Ensure cleanup on unexpected exit
        try:
            hb_path = Path(planboard_dir) / ".watcher_heartbeat"
            if hb_path.exists():
                hb_path.unlink()
        except Exception:
            pass
