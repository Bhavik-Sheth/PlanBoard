"""
planboard/tui/widgets/architecture_panel.py
...
class ArchitecturePanel(Static):
    ...
    def __init__(self, planboard_path: Path, **kwargs) -> None:
        super().__init__(**kwargs)
        self.planboard_path = planboard_path
        from planboard.watcher.watcher_manager import WatcherManager
        self.watcher_manager = WatcherManager(str(planboard_path))
    ...
    def refresh_diagram(self, path: Path | None = None) -> None:
        ...
        candidates = (
            [path] if path else [
                self.planboard_path / "ARCHITECTURE_DIAGRAMS" / "SystemArchitecture.md",
                self.planboard_path / "ARCHITECTURE_DIAGRAMS" / "SystemDesign.md",
            ]
        )
