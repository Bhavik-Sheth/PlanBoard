...
PlannerX is an AI-driven terminal project planner application designed to draft, refine, and structure product requirements (PRD), technical architecture documents (TRD), schemas, app flows, and coding rules in a single terminal interface (TUI).

Inspired by modern conversational tools like Claude Code and Gemini CLI, PlannerX provides a single conversational orchestrator agent that interprets your natural language messages, answers questions, manages the planning lifecycle, and automatically triggers specialist agents to write or refine documents.
...
2. **Interactive TUI Wireframe**: A full-screen Terminal UI built with Textual:
   - **FILE VIEW (Blue)**: Shows active `PLANNER/` file structure with reactive updates.
...
│  PLANNER/    │  - Agent logs & streaming stdout/stderr     │
...
```bash
git clone https://github.com/your-username/PlannerX.git
cd PlannerX
./install.sh
```
...
- **Initialize project**: `"initialize project"` scaffolds the `PLANNER/` directory.
...
- `/help` - Show help menu.
- `/init` - Scaffold `PLANNER/` directory.
...
- `/module add <name>` - Add a module specification under `PLANNER/MODULES/`.
...
| `Ctrl+C` / `Ctrl+Q` | Quit PlannerX cleanly |
