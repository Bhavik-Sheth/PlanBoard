...
### 🔴 CRITICAL — `prd_agent.py` Bypasses the `invoke_llm_safe` Retry Wrapper
...
- **Checklist**:
  - [ ] Remove `from planner.llm import get_llm` and `import os` from `prd_agent.py`
  - [ ] Add `from planner.agents._base import invoke_llm_safe, strip_markdown_fence` to `prd_agent.py`
  - [ ] Replace `get_llm()` + `llm.invoke(messages)` block with `invoke_llm_safe(messages)`
  - [ ] Replace manual fence stripping block with `strip_markdown_fence(prd_content)`
  - [ ] Run `python -c "from planner.agents.prd_agent import prd_agent"` to confirm clean import

---

### 🔴 CRITICAL — `prd_agent.py` Does NOT Set `state.calling_agent` on `needs_input`
...
- **Checklist**:
  - [ ] Add `state.calling_agent = "prd"` to the `needs_input` early-return block in `prd_agent.py`
  - [ ] Confirm by running: `python -c "from planner.state import PlannerState; from planner.graph import _route_from_griller; s = PlannerState(project_path='/tmp', calling_agent='prd', next_agent=''); print(_route_from_griller(s))"` → should print `prd`

---

### 🟠 HIGH — `scaffold.py` Scaffolds `CLAUDE.md` Inside `PLANNER/` but `finalize` Writes It to Project Root
...
- **Checklist**:
  - [ ] Remove `"CLAUDE.md"` from `PLANNER_FILES` list in `planner/files/scaffold.py`
  - [ ] Add an explanatory comment above `PLANNER_FILES` clarifying the exclusion
  - [ ] Optionally: add cleanup of stale empty `PLANNER/CLAUDE.md` inside `scaffold_project()`
  - [ ] Verify with: `python -c "from planner.files.scaffold import PLANNER_FILES; assert 'CLAUDE.md' not in PLANNER_FILES"`

---

### 🟠 HIGH — `watchfiles` Is a Runtime Dependency Declared Only in `[dependency-groups].dev`
...
- **Checklist**:
  - [ ] Move `watchfiles>=1.2.0` to `[project].dependencies` in `pyproject.toml`
  - [ ] Remove it from `[dependency-groups].dev`
  - [ ] Run `uv sync` to update `uv.lock`
  - [ ] Verify `watchfiles` appears in the main dependency list: `uv pip show watchfiles`

---

### 🟠 HIGH — Triple-Duplicated `_AGENT_MAP` in `app.py` (DRY Violation)
...
- **Checklist**:
  - [ ] Define `_FILE_AGENT_MAP` and `_resolve_agent()` at module level in `app.py` (or in a shared `planner/utils/agent_utils.py`)
  - [ ] Delete the 3 local `_AGENT_MAP = {...}` dicts from `_cmd_reset`, `run_reset`, and `_handle_change_request`
  - [ ] Replace each `_AGENT_MAP.get(...)` / manual prefix checks with `_resolve_agent(...)`
  - [ ] Also update `main.py`'s `_run_single_agent` to use the same shared map
  - [ ] Verify grep finds zero remaining `_AGENT_MAP = {` in `app.py`

---

### 🟠 HIGH — `reset` Action in Chat Orchestrator Dispatch Misses `MODULES/` and `ARCHITECTURE_DIAGRAMS/` Files
...
- **Checklist**:
  - [ ] Add `MODULES/` and `ARCHITECTURE_DIAGRAMS/` prefix checks to the reset dispatch in `_handle_change_request`
  - [ ] Add module name extraction (`target.split("/")[-1].replace(".md", "")`) when resetting a module
  - [ ] Test by sending chat: "reset MODULES/auth.md" and confirm agent is invoked
  - [ ] Test: "reset ARCHITECTURE_DIAGRAMS/SystemArchitecture.md" → confirm `generate_diagrams` is called

---

### 🟡 MEDIUM — `tracker_agent` Sets `next_agent="modules"` Causing Spurious Graph Node Invocation
...
- **Checklist**:
  - [ ] In `tracker_agent.py` line 77: change `state.next_agent = "modules"` to `state.next_agent = "orchestrator"`
  - [ ] Run the graph routing test: confirm `_route_from_specialist` with `tracker_agent` output routes to `orchestrator`, not `modules`
  - [ ] Optionally: add a comment explaining that `modules` is triggered explicitly by user, not from the main pipeline

---

### 🟡 MEDIUM — `planner/watcher/architecture_watcher.py` Is Orphaned Dead Code
...
- **Checklist**:
  - [ ] Confirm no production imports reference `planner.watcher` (grep check)
  - [ ] Delete `planner/watcher/architecture_watcher.py`
  - [ ] Delete `planner/watcher/__init__.py`
  - [ ] Remove the `planner/watcher/` directory
  - [ ] Run full import check: `python -c "import planner"` to confirm no broken imports remain

---

### 🟡 MEDIUM — `ViewerPanel.output_buffer` Grows Without Bound in Long Sessions
...
- **Checklist**:
  - [ ] Add `from collections import deque` to `viewer_panel.py`
  - [ ] Change `self.output_buffer = []` → `self.output_buffer = deque(maxlen=500)` in `__init__`
  - [ ] Verify `clear()` and iteration still work (they do natively on `deque`)

---

### 🟡 MEDIUM — `waiting_for_input` Boolean Is Not Thread-Safe
...
- **Checklist**:
  - [ ] Add `import threading` to `app.py` if not present
  - [ ] Replace `self.waiting_for_input = False` with `self._input_event = threading.Event()` in `__init__`
  - [ ] Replace all `self.waiting_for_input = True` → `self._input_event.set()`
  - [ ] Replace all `self.waiting_for_input = False` → `self._input_event.clear()`
  - [ ] Replace `if self.waiting_for_input:` → `if self._input_event.is_set():`

---

### 🔵 LOW — `pyproject.toml` Has Placeholder Description
...
- **Checklist**:
  - [ ] Update `description` field in `pyproject.toml` line 4

---

### 🔵 LOW — `ArchitecturePanel` Docstring Incorrectly Describes `.mmd` Rendering
...
- **Checklist**:
  - [ ] Update module-level docstring in `architecture_panel.py`
  - [ ] Update `ArchitecturePanel` class docstring
  - [ ] Update `refresh_diagram` method docstring to reference `.md` not `.mmd`

---

### 🔵 LOW — `structuring_agent` Hardcodes `next_agent="prd"`, Bypassing Orchestrator Routing
...
- **Checklist**:
  - [ ] In `structuring_agent.py` line 53: change `state.next_agent = "prd"` → `state.next_agent = "orchestrator"`
  - [ ] Verify the graph routing test still correctly transitions `structuring → orchestrator → prd`
